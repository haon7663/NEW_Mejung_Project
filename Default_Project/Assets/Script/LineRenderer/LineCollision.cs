...
public class LineCollision : MonoBehaviour
{
    ...
    void Update()
    {
        if(lc) colliderPoints = CalculateColliderPoints();
        if(polygonCollider2D) polygonCollider2D.SetPath(0, colliderPoints.ConvertAll(p => (Vector2)transform.InverseTransformPoint(p)));
    }
